...
LIBYUV_API
int NV21ToI420Rotate(const uint8* src_y, int src_stride_y,
  ...
  int halfwidth = (width + 1) >> 1;
  int halfheight = (height + 1) >> 1;
  if (!src_y || !src_vu || width <= 0 || height == 0 ||
	  !dst_y || !dst_u || !dst_v) {
	return -1;
  }
  ...
  switch (mode) {
	case kRotate0:
	  // copy frame
	  return NV21ToI420(src_y, src_stride_y,
	...
	case kRotate90:
	  RotatePlane90(src_y, src_stride_y,
					dst_y, dst_stride_y,
					width, height);
	  RotateUV90(src_vu, src_stride_vu,
			     dst_v, dst_stride_v,
				 dst_u, dst_stride_u,
				 halfwidth, halfheight);
	  return 0;
	case kRotate270:
	  RotatePlane270(src_y, src_stride_y,
					 dst_y, dst_stride_y,
					 width, height);
	  RotateUV270(src_vu, src_stride_vu,
			  	  dst_v, dst_stride_v,
				  dst_u, dst_stride_u,
				  halfwidth, halfheight);
	  return 0;
	case kRotate180:
	  RotatePlane180(src_y, src_stride_y,
					 dst_y, dst_stride_y,
					 width, height);
	  RotateUV180(src_vu, src_stride_vu,
			      dst_v, dst_stride_v,
				  dst_u, dst_stride_u,
				  halfwidth, halfheight);
	  return 0;
